 ...
 * （約 4 個遊戲日）。所以一株花的節奏是：
 *   種下（乾土）→ 澆水（濕土）→ 約 29 分（現實）土面乾掉 → 33 分冒「缺水」
 *   → 44 分還沒澆就開始枯 → 73 分枯死；不管枯不枯，長滿 **1 小時就完全開花可以摘**。
 * 也就是一輪花大概要澆兩次水。想更快看到效果就把數字改小。
 *
 * 用遊戲時間而不是現實時間有兩個好處：睡覺跳過的時間也算進去（睡一覺花就長大了），
 * 而且關掉遊戲再回來不會平白長大。
 */
export const GROW_MINUTES = 4930;      // 種下 → 完全開花（≈1 現實小時）
export const DRY_MINUTES = 3600;       // 最後一次澆水 → 開始枯萎（≈44 分）
export const WET_MINUTES = 2400;       // 澆水後土**看起來**還是濕的多久（≈29 分）
