 ...
 * （約 4 個遊戲日）。一株花的節奏是：
 *   種下（乾土，只有一個芽，**不澆水不會長**）
 *   → 澆水（土變深色，開始長）→ 澆下去算起 **1 小時完全開花，按 E 就能摘**
 *   → 一次澆的水撐 66 分（夠長到開花）；50 分時花圃冒「缺水」提醒
 *   → 沒補水就土乾＋開始枯萎，29 分內補澆還救得回來，再拖就枯死只能清掉。
 *
 * ⚠️ 一次澆的水（DRY）一定要**比長大所需（GROW）久**，不然種下澆一次水、乖乖等一小時
 * 的玩家會在開花前先看到花枯掉 —— 等於永遠摘不到。
 *
 * 用遊戲時間而不是現實時間有兩個好處：睡覺跳過的時間也算進去（睡一覺花就長大了），
 * 而且關掉遊戲再回來不會平白長大。
 */
export const GROW_MINUTES = 4930;      // 第一次澆水 → 完全開花（≈1 現實小時）
export const DRY_MINUTES = 5400;       // 澆水 → 水乾了開始枯萎（≈66 分，比長大久一點）
export const WET_MINUTES = 5400;       // 土看起來濕的時間＝水還在的時間（兩者一致才好懂）
